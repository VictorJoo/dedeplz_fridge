      ...
      <div class="container" >
        <div class="row">
            ...
            <table class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <th class="info text-center" width="10">HITS</th>
         ...
         <c:if test="${bvo.category=='공지사항'}">
             <td>${bvo.boardNo }</td>               
            <td>               
                     <a href="${initParam.root}showContent.do?boardNo=${bvo.boardNo }"><font color="black">★[${bvo.category}] ${bvo.title}</font></a>
            </td>
            <td>${bvo.nick }</td>
            <td>${bvo.postDate }</td>
            <td>${bvo.hits }</td>
   ...
   <div class="col-md-12 text-center">
             <ul class="pagination">
               ...
               <c:if test="${requestScope.lvo.pagingBean.previousPageGroup}">
                  <li><a
                     href="${initParam.root}searchByCategory.do?pageNo=${requestScope.lvo.pagingBean.startPageOfPageGroup-1}">Prev</a>
                  </li>
               </c:if>
               <c:forEach var="i"
                  ...
                  <c:choose>
                     <c:when test="${requestScope.lvo.pagingBean.nowPage!=i}">
                        <li><a              
                           href="${initParam.root}searchByCategory.do?pageNo=${i}">${i}</a>
                        </li>
                     </c:when>
                     <c:otherwise>
                        <li><a href="#">${i}</a></li>
               ...
               <c:if test="${requestScope.lvo.pagingBean.nextPageGroup}">
                  <li><a
                     href="${initParam.root}searchByCategory.do?pageNo=${requestScope.lvo.pagingBean.endPageOfPageGroup+1}">Next</a>
                  </li>
               </c:if>
            </ul>
            </div>

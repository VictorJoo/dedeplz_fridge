	...
	<div class="col-md-12 text-center">
 				<ul class="pagination">
					...
					<c:if test="${requestScope.lvo.pagingBean.previousPageGroup}">
						<li><a
							href="${initParam.root}BoardList.do?pageNo=${requestScope.lvo.pagingBean.startPageOfPageGroup-1}">Prev</a>
						</li>
					</c:if>
					<c:forEach var="i"
						...
						<c:choose>
							<c:when test="${requestScope.lvo.pagingBean.nowPage!=i}">
								<li><a
									href="${initParam.root}BoardList.do?pageNo=${i}">${i}</a>
								</li>
							</c:when>
							<c:otherwise>
								<li><a href="#">${i}</a></li>
					...
					<c:if test="${requestScope.lvo.pagingBean.nextPageGroup}">
						<li><a
							href="${initParam.root}BoardList.do?pageNo=${requestScope.lvo.pagingBean.endPageOfPageGroup+1}">Next</a>
						</li>
					</c:if>
				</ul>
				</div>
